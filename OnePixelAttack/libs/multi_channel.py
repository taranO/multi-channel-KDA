...
import libs.mobilenetv2 as modelmobilenetv2
...
class MultiChannel(nn.Module):
    def __init__(self, model_name="vgg16", MODEL_DIR="", EPOCHS=[], P=[1,2,3],SUBBANDS=["d", "h", "v"],
                 IMAGE_SIZE=32, N_CHANELS=3, use_cuda=True, is_prefilt=False):
    ...
    def forward(self, x):
        ...
        for i in range(N):
            ...
            inputs = torch.FloatTensor(self.applyDCTPermutation(z.copy(), self.permutation[i]))

            if self.use_cuda:
                inputs = inputs.cuda()

            inputs = Variable(inputs, volatile=True)
            pred_labels += self.NET[i](inputs).data.cpu().numpy()
